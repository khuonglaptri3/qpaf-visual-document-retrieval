...
- [ ] Configure repository-local author from the verified GitHub identity.
- [ ] Commit the bootstrap on `main`.
- [ ] Run `git switch -c develop`, then `git switch -c feature/m1-repository-audit`.
- [ ] Check `git branch` and `git status --short`; expect the three branches and a clean feature checkout.
...
- [ ] Write CLI tests with temporary files before implementation. Use literal SHA-256 for bytes `abc`: `ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad`.
- [ ] Run `python -m unittest discover -s tests -v`; expect failures because the CLI is not implemented.
- [ ] Implement the interface and safeguards in the spec. Inventory columns are `path`, `category`, `size_bytes`, `sha256`, `status`, `note`; specialised inventories retain those columns.
- [ ] Run the same complete suite; require no failures before committing.
...
- [ ] Add dated task tracking, ownership notes and corpus/alias/duplicate CSV schemas.
- [ ] Write OCR routing/calibration/failure and artifact namespace policies without invented numerical thresholds.
- [ ] Commit implementation and preparation before taking the audit snapshot.
